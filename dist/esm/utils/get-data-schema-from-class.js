...
export function getDataSchemaFromClass(ctor, doNotThrowIfNoMetadata = false) {
    const metadata = getDataSchemaMetadataFromClassAndProperties(ctor, doNotThrowIfNoMetadata);
    return convertDataSchemaMetadataOrClassFactoryToDataSchema(metadata, doNotThrowIfNoMetadata);
}
/**
 ...
 * @param doNotThrowIfNoMetadata
 */
function convertDataSchemaMetadataOrClassFactoryToDataSchema(input, doNotThrowIfNoMetadata = false) {
    if (typeof input === 'function')
        return getDataSchemaFromClass(getClassFromFactory(input), doNotThrowIfNoMetadata);
    const result = { type: DataType.ANY };
    ...
    if (input.type === DataType.OBJECT) {
        ...
        if (typeof input.properties === 'function') {
            result.properties = getDataSchemaFromClass(getClassFromFactory(input.properties), doNotThrowIfNoMetadata).properties;
        }
            ...
            if (propNames.length) {
                ...
                propNames.forEach(propName => {
                    ...
                    result.properties[propName] =
                        convertDataSchemaMetadataOrClassFactoryToDataSchema(propValue, doNotThrowIfNoMetadata);
                });
            }
        }
        ...
        if (typeof input.items === 'function') {
            result.items = getDataSchemaFromClass(getClassFromFactory(input.items), doNotThrowIfNoMetadata);
        }
        ...
        // то выполняется рекурсия на случай наличия фабрики
            ...
            result.items = convertDataSchemaMetadataOrClassFactoryToDataSchema(input.items, doNotThrowIfNoMetadata);
        }
 ...
 * @param doNotThrowIfNoMetadata
 */
function getDataSchemaMetadataFromClassAndProperties(ctor, doNotThrowIfNoMetadata = false) {
    ...
    if (typeof result.properties === 'function') {
        result.properties = getDataSchemaFromClass(getClassFromFactory(result.properties), doNotThrowIfNoMetadata).properties;
    }
    ...
    for (const [propName, propMd] of propsMd) {
        if (propMd)
            result.properties[propName] = propMd;
    }
    return result;
}
/**
 ...
 * @param factory
 */
function getClassFromFactory(factory) {
    const cls = factory();
